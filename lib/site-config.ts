 ...
 * switch is not rendered at all, and every rule in app/page-ideas.css targets
 * only an `.idea-` class that markup carries (scripts/page-ideas.test.mjs
 * enforces that).
 *
...
export const pageIdeas: Record<
  "projectsFade" | "notFoundPrefill" | "postsLead" | "noteReadAloud",
  boolean
> = {
  ...
  /** Posts: the newest post leads the page with its picture and opening (#180). */
  postsLead: true,
  /** Post pages: "Listen" reads the note aloud and follows along (#180). */
  noteReadAloud: true,
};
