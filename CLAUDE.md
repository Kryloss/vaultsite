...
- **Contents pill on phones**: below 640px the ToC pill sits in the TOP-RIGHT corner as a three-line icon and unfurls the current heading only while scrolling down, opposite the breadcrumb→time swap in the left bar. Driven by `html[data-scroll]`, written by `Chrome` and read by CSS (#50, #51). Unchanged from 640px up.
