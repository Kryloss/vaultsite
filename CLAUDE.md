...
- **Reading time**: `readingStats()` shown on posts-type entry pages only; the same `minutes` is passed to `ReadingProgress`, which shows a "N min left" pill bottom-right once you're 6% in (#46).
