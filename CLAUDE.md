...
- **Series** (`lib/series.ts` + `components/Series.tsx`): `series: Road to Security+` in an entry's frontmatter joins it to a multi-part arc — a "Part 2 of 5" badge beside the date which opens a popover listing every part (built like the mobile ToC sheet: always mounted, `data-open`, `inert` when closed, contents gated behind first open; a bottom sheet below 640px). Matching is by name (case/space-insensitive) and **vault-wide**, so a series can run across sections; no route or URL changes. Order is oldest-first, overridable per note with `part: 2` (for same-day or backdated parts). `series_uk:` is written on any ONE part and used for all. A series with a single published part renders nothing. See `docs/DECISIONS.md` #42.
