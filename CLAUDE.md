...
- **Series** (`lib/series.ts` + `components/Series.tsx`): `series: Road to Security+` in an entry's frontmatter joins it to a multi-part arc — "Part 2 of 5" beside the date, and a panel listing every part above the prev/next row. Matching is by name (case/space-insensitive) and **vault-wide**, so a series can run across sections; no route or URL changes. Order is oldest-first, overridable per note with `part: 2` (for same-day or backdated parts). `series_uk:` is written on any ONE part and used for all. A series with a single published part renders nothing. See `docs/DECISIONS.md` #42.
