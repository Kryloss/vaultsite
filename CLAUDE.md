...
- **Reading position** (`components/ReadingPosition.tsx`): remembers how far into a note you got and *offers* to return — never scrolls on its own. One localStorage key, map of path → position, pruned to 20 notes / 30 days, written on a debounce rather than per scroll frame.
...
- **Motion** (`docs/DECISIONS.md` #38, #44): `.stagger` on a list container makes its children arrive one at a time (nth-child delays, capped at 12; `animation: none` under reduced motion and print, since a shortened duration still honours the delay). Navigations run through the View Transitions API (`components/PageTransitions.tsx` + `lib/view-transition.ts`) — forward and back animate differently via `html[data-nav]`, and a list row's title morphs into the entry's `<h1>` (`data-vt-title` → `.entry-title`); the lightbox opens as a zoom out of the figure you clicked. All of it is skipped where the API is missing or under reduced motion, falling back to the `.page-in` fade. Shelf rows are drag-to-scroll with hover arrows (`components/lists/ShelfRow.tsx`; the old edge fade is gone). The ToC rail highlight is one sliding `.toc-marker` positioned from the live DOM. Dialogs (palette, shortcut sheet, ToC sheet) stay mounted and toggle `data-open` so they animate shut, with `inert` when closed and contents gated behind first open to keep them out of every page's HTML.
