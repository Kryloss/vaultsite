...
- **There is no permanent rail at any width.** The sidebar is a slide-in drawer, closed by default, opened from a floating pill at the top-left that also carries the breadcrumb. The drawer is a modal dialog: focus trapped while open, restored on close, `inert` when closed.
