...
- **Every pill, badge and card is ONE material** — the breadcrumb bar's `--chrome-bg` fill and inset `--chrome-ring` hairline (#185). Floating chrome wears `.chrome-bar` (below). In the page: `.pill` for a pressable control (category chips, /music's search and language button, Now's "Updated" and "PDF") at the same 2.5rem and 14px medium `--text-secondary`, selected = `.is-active`, the `--text` inversion; `.pill-tone` for the material alone on a small label that keeps its size; `.card` for cards (Explore, the Now goals, the Posts lead). Over a picture it also blurs (a person's name card, the cover badges). The block is the LAST in `globals.css`, unlayered — so it beats the Tailwind utilities, and a `hover:bg-…` utility on one of these does nothing: hover is written in that block. The amber Draft chip (a dev-only warning) and the lightbox's round arrows are the deliberate exceptions.
- **Every floating chip wears `.chrome-bar` itself** — the breadcrumb bar, the Today's vibe capsule, the time remaining (`.time-left`), "Continue" (`.resume-reading`), the read-aloud player, the selection link and the dev pencil — at its 2.5rem height, its 14px medium `--text-secondary` type, 0.75rem off the window's edge and its z-index 30 (#167, #184). A new chip takes the class; it does not copy the fill, ring and blur, which is how the bottom ones drifted to 34px, 13px and a flat blur.
