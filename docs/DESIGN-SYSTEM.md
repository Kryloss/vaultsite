...
- **Every floating chip wears `.chrome-bar` itself** — the breadcrumb bar, the Today's vibe capsule, the time remaining (`.time-left`), "Continue" (`.resume-reading`) and the read-aloud player — at its 2.5rem height, its 14px medium `--text-secondary` type, 0.75rem off the window's edge and its z-index 30 (#167, #184). A new chip takes the class; it does not copy the fill, ring and blur, which is how the bottom ones drifted to 34px, 13px and a flat blur.
