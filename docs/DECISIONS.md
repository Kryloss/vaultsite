...
**Every field below the name degrades on its own.** No `author_photo:` falls back to initials on the card surface; no `author_bio:` leaves the role and the name standing. That is not a rare path, it is half the shelf: a novelist usually has a freely licensed portrait on Wikimedia Commons and a YouTube channel usually has none. Four of the fifteen notes ship on initials, so both shapes are in the build and neither is theoretical. `author:` stays the only required key, which is why adding the block to the site invalidated no existing note.
...
**A row, not a card.** No border, no fill, no radius. The At a glance table directly below is already a framed block, and two of them stacked at the top of an article read as a dashboard rather than the opening of something you're about to read. A hairline and space separate it, which is the pair the entry footer already uses.
