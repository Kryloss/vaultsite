...
**The sentence sits ON the number, and the fade is what makes that legible.** The first line rides over the tail of the glyphs rather than clearing it, so the two read as one block instead of a heading with a caption under it — a lift of 0.28 of the number's own size, held in `--notfound-size` on the wrapper, because a fixed rem clears the digits at one end of the clamp and crosses their middles at the other. That forced a third stop into the gradient: a straight two-stop fade is still around a third of the text colour where the words cross it, which reads as a smudge through them, so the fade is held back to 62% and then dropped steeply — solid where nothing is over the digits, nearly gone where something is. Nothing needs a z-index: the sentence comes after the number in the DOM, and `.stagger > *` gives every child its own stacking context.
