...
**The shelf is a SCROLLER, not a wrapping grid.** Wrapping was the original behaviour and it turned a shelf into a bookcase of stacked rows the moment the books got large; a shelf is one line of books you look along. It now uses the same `ShelfRow` the other mediums use — same drag, same grab cursor, same hidden scrollbar — which is why the `<ul>` carries `.shelf-row` as well as `.book-shelf`: the dragging rules are scoped to that class and reimplementing them would be two things to keep in step.

**And the padding that buys the cover its room steals the heading's clicks.** Padding takes pointer events like any other part of an element, so a row whose box starts ~160px above where its books appear sits squarely over its own "Books" link — swallowing the click and showing a grab cursor on the title. `.book-shelf-heading` raises the heading above it. Raising the heading is the fix rather than shrinking the padding, because the padding is the cover's only room; and a hover cover still paints over the heading when one rises that far, which is correct for a transient popover. **Any row that reserves overflow room this way will do the same to whatever it overlaps** — check what a click at the heading actually lands on, not just how it looks.

**Scrolling costs the hover cover its escape route.** `overflow-x: auto` makes `overflow-y` compute to `auto` as well — `visible` is not a legal pairing — so the face that rises ABOVE a book is clipped at the row's top edge. The row therefore carries `padding-top: var(--cover-reach)` and hands the same amount back in `.book-shelf-gap`'s margin, so the books do not move. `--cover-reach` is the cover's height plus 1.75rem, and every part of that was measured against the worst case rather than reasoned about: the reading book is `scale(1.045)` from its foot, standing ~18px above its own layout box and carrying its cover up with it, and hover lifts it 8px further. 0.5rem clipped it by 9px; 1.25rem still clipped it by 1. **A 1px clip is invisible and still wrong** — check it by forcing the covers visible and comparing their tops to the row's.
...
Every section page printed `main.md`'s body in one place: under the title, above the list. That is right almost everywhere — the prose introduces the page, so it comes before the page. Music is the exception, because the first thing in its list is not a list at all: it is the Apple Music player, and the paragraphs are a note ON that player and on the notes below it. Printed above the embed they were an introduction to an introduction, and the reader had to scroll past the writing to reach the thing the page exists for.

**The prose moved, not the player.** The alternative was to push the embed down inside `MusicList`, which is the same rearrangement written the other way round — and worse, because the embed would then sit in the middle of the page with nothing above it but a title. What the page wants is: title, the music, then what I think about it.

**One flag, and the page keeps ownership of the markdown.** `ListProps` gained an optional `body`, and `listRendersBody(type)` in `lib/section-types.tsx` answers whether a type places it itself. `app/[section]/page.tsx` still does all the rendering — both languages, both `<article>` elements, `previewsInHtml` unchanged — and simply hands the finished node down instead of printing it, so the two paths cannot render different HTML. A type that doesn't opt in gets `undefined` and behaves exactly as before. The `bodyBelow` set is deliberately a set rather than a boolean on the component: the decision is about the PAGE's order, so it belongs beside the registry that owns page order, not inside a component that would have to be read to discover it.
...
**Fifth pass: the flex line sets the height, and the picture fills it.** A fixed width let the ARTWORK'S SHAPE decide the block: a 2:3 poster stood 1.5× its width and reached the metadata line, while a SQUARE portrait was only as tall as it was wide and left a stripe of empty column under it. Sizing by height instead — width from the aspect ratio — was tried first and is the wrong end of the problem: a wider box takes the room the title wraps in, so the text got TALLER as the picture grew and the gap opened again (a square portrait went from 48px against 67px of text to 69px against 102px). The width has to stay put; only the height can give. So `.note-header` is `align-items: stretch` and `.note-thumb` has `height: auto`: the box is as tall as the title and its metadata line, whatever shape the picture is, and `object-fit: cover` fills it. Measured after: 48 × 67 against 67px of text on a phone, 48 × 82 against 82 on a laptop — flush at both.

**This is the one place on the site where artwork is cropped to a frame**, and every other cover, spine and poster keeps its own ratio (#110, #115). The trade is deliberate and small: at 48px this is a label, not the artwork, and a portrait cropped a few pixels top and bottom still reads as the person while a stripe of empty column reads as a bug. A three-line title would stretch it into a sliver, so it stops at twice its width and the space goes back to being space — beside a block that tall, nobody is looking for it.
