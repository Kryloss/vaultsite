...
**A week's own summary sits UNDER the bars** — "16.07 · 14n", in its own reserved line — rather than sharing the line above. Near the thing it describes, and out of the way of the range and total. Abbreviated because it has a 176px column and two things to say; the panel spells both out in full once a week is actually opened, and the button's `sr-only` name keeps the long sentence, so a screen reader never hears "sixteen dot zero seven, fourteen en". Its line is reserved by the section's `padding-bottom` for the same reason as the one above: a line that appeared on hover would push the social icons down every time the pointer crossed a bar.
