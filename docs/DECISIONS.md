...
**Tested and rejected (2026-07-18):** The embed reportedly works in a fresh browser but stalls after repeated visits — pointing at stale third-party storage the player accumulates. A *storage-isolating* sandbox (`allow-scripts allow-popups allow-forms allow-presentation`, deliberately **without** `allow-same-origin`) was tried to force a clean-slate "first visit" every load. Verified on production: it leaves the player permanently gray — Apple's embed *requires* same-origin storage to hydrate at all. Reverted. The stale-storage theory may still hold, but the only reset lever is the user clearing site data for `kryloss.com` **and** `music.apple.com` (browsers partition third-party storage by top site) — nothing our page can do cross-origin.
**Revisit when:** Apple ships a reliable embed load/error event we can key an auto-retry on, or the stalls stop.
