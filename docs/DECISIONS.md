...
**Higher and smaller in the gutter, and the width is what was actually wrong.** 9rem started the player level with the note's body while the title sat above it, so it read as pinned to the wrong thing; 7rem lines it up with the top of the page's own content. 16rem instead of 18rem also buys back the margin the previous entry (#94) had spent — 128px to the window edge at 1440px rather than 96px. The 1400px breakpoint stays: the narrower player would now fit at 1280px, and lowering it is a one-line change, but that is a claim about how Apple's player renders at 256px and this session could not see it render at all.

**Still unverified, and the same caveat as #94: Apple's embed served only its gray skeleton throughout.** Every structural fact here is measured — the pill is `flex` below 640px and `none` above, the inline album is `none` on a phone, the sheet opens with `inert` cleared and its iframe mounted at 450px, the sheet's top edge meets the pill's bottom edge exactly, the desktop player is 256px at `top: 7rem` with no overflow — but **what the player looks like inside those boxes is not something this session could confirm.**
