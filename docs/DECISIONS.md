...
The 404 opened like every other page — a 24px "404", a line of grey under it, a text link — which is the layout of a page that has something to say applied to the one page that has nothing. It is now the number, masked out down its own height at roughly twice `.page-title`, two ways out under it, and the "did you mean" guess below a rule. Nothing else: no sentence, no glow, no third control.
...
**The fade is a MASK, and the version before it is worth recording.** The first build painted a gradient into the glyphs with `background-clip: text`, `--text` down to `color-mix(… 30%, transparent)`. It worked, and it cost three things: the fill has to be transparent for a clipped background to show, so a browser without `background-clip: text` renders an INVISIBLE number rather than an unfaded one — an `@supports` around it, and a `forced-colors` escape inside that to put `CanvasText` back. And because it faded to ink rather than to nothing, the sentence that then lay across it needed a third gradient stop to stop the strokes smudging the words. `mask-image` has none of those: the fill stays `--text`, so selection and forced colours are right without being mentioned; unsupported, it simply doesn't fade, which is a number rather than a blank; and it goes to fully transparent, which is what makes the overlap legible. Still nothing about it is a colour (#64) — it takes the digits out towards whatever the page is behind them, in either theme, with one declaration. The prefixed line is kept beside the plain one: Safari read `-webkit-mask-image` alone until 15.4. It ends at 96% because the baseline of a line box this tight sits at about 89%, and a mask finishing earlier amputates the feet of the digits instead of fading them.

**Everything on it is one step quieter than the first build.** The number's ceiling came down from 10rem to 7.5rem, the two controls lost a size and a radius step, the rhythm tightened, and in the end the sentence went entirely — the page read as overwhelming with all of it at full volume, which is a strange thing for a page whose whole content is one number and a sentence. The number is still roughly twice `.page-title` and still the biggest thing on the site; it just no longer arrives with a filled button and a field at the same weight underneath it.

**A pool of light behind the number was built, and it is out.** Two versions of it: one soft radial, then a tight core inside a wide halo at two per-theme strengths, because ink at 17% on black is a glow while the same mix on white is a smudge. Removed at the owner's request — the gradient down the digits is the effect, and the page is flat behind them. Three things learned there are worth keeping, since a glow is the obvious thing to reach for again. It has to be `z-index: -1` on the WRAPPER's pseudo-element: a positioned pseudo paints after its non-positioned siblings, so it lands on top of the number otherwise, and putting it on the number instead is the same bug one element further in. Its width has to be capped at the column's own — an absolutely positioned box reaching past the viewport widens the DOCUMENT, and 130% of the hero was 451px of it in a 390px window, scrolling a phone sideways. And with the glow gone the mask's ramp had to lengthen (8% rather than 28%): a short ramp under a long solid top reads as a number that has been cut off rather than one going out, which only became visible once nothing else was lighting it.

**The sentence under the number is gone too, and its tuning is worth keeping.** "This page doesn't exist." was first set below the number, then tucked INTO it so its first line rode the tail of the glyphs — the `-mt-8` of the component this was adapted from, rewritten as a fraction of the number's own size, because the number is fluid and a fixed rem clears the digits at one end of the clamp and crosses their middles at the other. Two numbers made that work and one of them had to be found twice: the lift is a GRAZE at 0.15, not 0.28 — 0.28 put both lines across the middle of the digits where the strokes are solid, a collision rather than an overlap, and it was the first thing anyone said about the page — and the mask has to be gone by the digits' feet rather than merely faint there, or the stems of the 4s run through the words. Then the sentence was deleted outright: "404", under a breadcrumb that already names where you are, needs no gloss, and the page is now a number and two buttons. `notFoundBody` is out of `ui` with it. Anything that lands under the number again starts from those two numbers.

**Two ways out, matched: Home and Search.** Neither is louder than the other. Home first took the inverted fill the active nav row and the selected chip use, and that made it the brightest thing on a page that is already a very large number — two quiet surfaces read as a pair of doors rather than as an instruction, and a shared min-width keeps them the same size instead of sizing each by the length of its own word. The arrow on Home is the site's own `.arrow-glyph`, and Search carries `SearchIcon` — **the one icon on page content anywhere on the site**, added at the owner's request after the pair went grey. It is doing the arrow's job on the other button: with two identical boxes side by side, the mark is what tells them apart before the words are read. That is a deliberate hole in #64, recorded in `docs/DESIGN-SYSTEM.md` beside the rule it breaks; a second one would end the rule rather than bend it. The reference this was adapted from still loses its house and compass — its icons were decoration on labelled buttons that already read as buttons. Search replaced a link to `/posts`: on the page you land on when the URL was wrong, the useful control is the one that finds the right page, not another list to browse.
...
Opening the lightbox used to move the `lightbox-figure` name from the thumbnail to the overlay's copy, and a named element is cut out of the page snapshot, so the small version vanished for the length of the zoom and popped back once the live page took over. The owner asked for it to stay. Now the thumbnail never carries the name on the way in: an empty `position: fixed` box laid over its drawn rect (hover scale included) does, and is removed in the update. The zoom only needs that box's geometry, because `::view-transition-old(lightbox-figure)` is hidden anyway, and the real thumbnail stays in the root snapshot and takes the page's veil with it. Closing mirrors it: the thumbnail used to take the name after the overlay was gone, which cut it out of the new page snapshot, so it faded out with the old page before the picture landed. Now a stand-in over it takes the name, `html.lightbox-closing` shows the overlay's picture (the "before" side) shrinking onto it, and the thumbnail fades from dimmed to normal with the page underneath. Tall diagrams whose overlay box is letterboxed may land a touch small before the handover; images keep their ratio exactly. The caption rides along as `lightbox-caption` by the same method, its box taken from a Range over the figcaption's text rather than the full-width block, because the overlay caption is shrink-wrapped and two boxes of different shapes would stretch the text in flight. Both captions, `.prose figcaption` and the overlay's, share a new token, `--text-caption`, at the owner's request, so the text is one colour from the page to the lightbox and back: in dark mode `#b5b5b6`, the overlay's old white at 70% flattened onto `--bg` (the owner preferred it to the tertiary grey; kept opaque because two translucent captions overlapping mid-transition flashed lighter) and `--text-tertiary` in light mode, where white would vanish on the page. Before, the overlay's white was invisible over a light page until the page had darkened, and read as arriving already in place. A copy of the page caption cross-fading into the overlay's, with the page caption hidden meanwhile, was tried the same day and dropped: the owner wanted one colour and the page caption left alone. The two groups' timing is literal (320ms, the `--ease` curve), not `var()`, like `lightbox-veil`, because Safari doesn't resolve custom properties on transition pseudo-elements. Stepping with ← / → now moves the close target to the figure on show; before, closing after a step shrank one picture onto another's thumbnail. The arrows and counter used to appear sharply at the handover (they were only in the hidden new page snapshot); they now carry their own names, `lightbox-prev` / `-next` / `-count`, keep the engine's fade (in on open, out on close) at the zoom's length, and `travel()` animates their groups with `element.animate({ pseudoElement })` from their spot mapped onto the thumbnail at its scale to where they really are, and back on close — so they move the way the picture and caption do. Timing is written out in TypeScript too (`ZOOM_TIMING`); `withViewTransition()` gained an optional `ready` callback for it.
...
## 178. A design 2.0 was built, tried and fully reverted (2026-09-23)

The owner asked for a "version 2.0" step up in experience and recognisability, easy to revert. It was built on branch `design/v2` behind a one-word switch (`siteDesign` in `lib/site-config.ts`, every 2.0 style scoped to `html[data-design="2"]`): display-size titles and greeting; a "Currently" board on home (listening, reading, building, next goal, read from the vault); a numbered Index of every section with picture previews in place of Explore; a colophon footer on every page ending in a large fading signature of the chevron mark and the name; the mark and note counts in the drawer; "N notes · Updated …" under section titles. A second, smaller branch `home/currently` kept only "Currently" and picture previews on the Explore cards, and removed the "Curious what I'm up to now?" line from the home note.
