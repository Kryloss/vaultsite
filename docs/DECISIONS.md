...
**Two ways out, and the second one is search.** Home takes the inverted fill the active nav row and the selected chip already use, with the site's own `.arrow-glyph` on it — icons stay in the sidebar (#64), so the reference this was adapted from loses its house and compass and keeps its shape. Beside it is the search field, which replaced a link to `/posts`: on the page you land on when the URL was wrong, the useful control is the one that finds the right page, not another list to browse.

**That field is a BUTTON dressed as a field.** There is one search on this site — the ⌘K palette, with its index, its keyboard model and its results list — and a real `<input>` here would be either a second, worse one or a box that throws away what you typed the moment the palette opened over it. Pressing it opens the palette, which is where the cursor lands anyway. The palette's open state belongs to `Chrome`, which is not an ancestor of page content, so the request travels as an `opensearch` window event — the same shape as the palette asking `Shortcuts` for the shortcut sheet, and as `TIME_LEFT_EVENT`. It also warms the index on pointer-enter, which matters precisely on the 404s that suggest nothing: those are the ones where someone reaches for search.
