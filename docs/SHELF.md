...
| `cover:` | Resolved by file name anywhere in the vault. YouTube videos derive a thumbnail from `video:` instead; an Instagram reel or a TikTok has none to derive, so its note carries a `cover:` saved into `Videos/covers/` (#171). |
| `video:`, `uploaded:` | Video notes. `video:` is a YouTube, Instagram or TikTok link (the full address — a `vt.tiktok.com` short link is not resolved). `uploaded:` is the VIDEO's publish date and is what makes a YouTube note a `VideoObject`; without it the markup is deliberately not emitted, and an Instagram or TikTok note stays a `CreativeWork` either way. New videos: `docs/YOUTUBE-SHELF.md`. |
...
- Hover floats the book's face above it (`.book-spine-cover`, 6rem). It is `pointer-events: none`; its z-index sits on `.book-slot` (the stagger animation gives every slot a permanent stacking context); `@media (hover: none)` uses `display: none` so phones fetch no covers. `.book-spine` has `position: relative` and no `overflow: hidden`; the row carries `padding-top: var(--cover-reach)` and `.book-shelf-heading` is raised above it so the heading stays clickable. The cover is centred on its spine except on the first and last slot, which anchor it to the spine's outer edge (`--cover-left`/`--cover-shift`): a centred cover overhangs ~15px each side and the scroller clips horizontally.
