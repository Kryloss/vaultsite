...
| `cover:` | Resolved by file name anywhere in the vault. YouTube videos derive a thumbnail from `video:` instead; an Instagram reel has none to derive, so its note carries a `cover:` saved into `Videos/covers/` (#171). |
| `video:`, `uploaded:` | Video notes. `video:` is a YouTube or Instagram link. `uploaded:` is the VIDEO's publish date and is what makes a YouTube note a `VideoObject`; without it the markup is deliberately not emitted, and an Instagram note stays a `CreativeWork` either way. New videos: `docs/YOUTUBE-SHELF.md`. |
